            ...
            <div className='flex flex-col items-center'>
                {sessions.length > 0 ? (
                    sessions.map((session) => {
                        return (
                            <div key={session.id} className='w-[90%] flex shadow shadow-[#94a3b8] rounded-md m-2'>
                                <div className='flex flex-col m-4 pr-4 flex-grow'>
                                    <h5><span className='text-[#00a396] text-base'>{session.estado}</span></h5>
                                    <p className='text-[#94a3b8] text-xs'>Fecha: <span className='text-[#89898a]'>{session.fecha || 'No disponible'}</span></p>
                                    <p className='text-[#94a3b8] text-xs'>Hora: <span className='text-[#89898a]'>{session.hora || 'No disponible'}</span></p>
                                    <p className='text-[#94a3b8] text-xs'>Duracion: <span className='text-[#89898a]'>{session.duracion || 'No disponible'}</span></p>
                                    <p className='text-[#94a3b8] text-xs'>Tipo de sesión: <span className='text-[#89898a]'>{session.tipoSesion || 'No disponible'}</span></p>
                                    <p className='text-[#94a3b8] text-xs'>Observaciones: <span className='text-[#89898a]'>{session.observaciones || 'No disponible'}</span></p>
                                </div>
                    ...
                    <div className='w-[90%] bg-[#f6e9e6] border border-red-300 rounded-md text-[#FF6F59] m-4 p-4'>
                        <ErrorOutlineTwoToneIcon className='mr-2'/>
                        {error}
                    </div>
                )}
